    ...
    public class SAP2DAgent : MonoBehaviour
    {
        ...
        private void Rotate(Vector3 dir)
        {
            ...
            if (dir.x < 0)
            {
                ...
                CheckEnemyAround(Vector2.left);
            }
            if (dir.x > 0)
            {
                transform.localScale = new Vector2(-1, transform.localScale.y);
                CheckEnemyAround(Vector2.right);
            }
                
            transform.rotation = Quaternion.RotateTowards(transform.rotation, q, Time.deltaTime * RotationSpeed);
        }

        public void CheckEnemyAround(Vector2 directionRay)
        {
            float rayDist = 2f;
            RaycastHit2D horizontalHit = Physics2D.Raycast(checkEnemy.transform.position, directionRay, rayDist, enemyMask);
